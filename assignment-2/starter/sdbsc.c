...
{
    if (id < MIN_STD_ID || id > 1000000) {  // changed here!
        return SRCH_NOT_FOUND;
    }
    
    if (lseek(fd, (id - 1) * STUDENT_RECORD_SIZE, SEEK_SET) == -1) {
...
int del_student(int fd, int id)
{
  student_t student;
    ...
    // prepare an empty student record
    ...
    ssize_t bytes_written = write(fd, &empty, STUDENT_RECORD_SIZE);
    if (bytes_written != STUDENT_RECORD_SIZE) {
        fprintf(stderr, M_ERR_DB_WRITE);
        return ERR_DB_FILE;
    }
...
void print_student(student_t *s)
{
    ...
    // Print the header using the expected column names.
    printf(STUDENT_PRINT_HDR_STRING, "ID", "FIRST_NAME", "LAST_NAME", "GPA");
...
{
    // Open a temporary database file; truncate if it already exists.
    ...
    // Read records from original file and write valid ones to temporary file.
    while (read(fd, &student, STUDENT_RECORD_SIZE) == STUDENT_RECORD_SIZE) {
        if (student.id != 0) {
